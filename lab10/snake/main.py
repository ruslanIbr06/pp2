...
# Function to fetch or create a user
def get_or_create_user(username):
    cursor.execute("SELECT id, score, level, speed FROM users WHERE username = %s", (username,))
    user = cursor.fetchone()

    if user:
        user_id, score, level, speed = user
    else:
        cursor.execute("INSERT INTO users (username) VALUES (%s) RETURNING id", (username,))
        user_id = cursor.fetchone()[0]
        score, level, speed = 0, 1, 150
        conn.commit()

    return user_id, score, level, speed

# Function to update score in the database
def update_user_score(user_id, score, level, speed):
    cursor.execute("""
        INSERT INTO user_score (user_id, score, level, speed)
        VALUES (%s, %s, %s, %s)
        ON CONFLICT (user_id) 
        DO UPDATE SET score = EXCLUDED.score, level = EXCLUDED.level, speed = EXCLUDED.speed
...
user_id, score, level, speed = get_or_create_user(username)
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYDOWN:
            ...
            if event.key == pygame.K_p:  # Pause shortcut
                ...
                pygame.event.clear()  # Stop the event queue temporarily

    # Collision with self
    for seg in coor_body[:-1]:
        if coor_head == seg:
            game_over_message("times new roman", 50, (255, 0, 0))
            update_user_score(user_id, score, level, speed)
            done = True
